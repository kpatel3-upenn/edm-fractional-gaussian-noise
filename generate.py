...
def partial_denoising_sampler(net, latents, class_labels=None, randn_like=torch.randn_like,
    ...
    # Time step discretization.
    step_indices = torch.arange(end_step - start_step, dtype=torch.float64, device=latents.device)
...
def partial_denoising_sampler(net, latents, class_labels=None, randn_like=torch.randn_like,
    ...
    # Main sampling loop.
    x_next = latents.to(torch.float64)
    for i, (t_cur, t_next) in enumerate(zip(t_steps.flip(0)[:-1], t_steps.flip(0)[1:])):  # Start step, ..., End step - 1
        x_hat = x_next
        t_hat = t_cur
